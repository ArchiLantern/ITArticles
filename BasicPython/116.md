...
第一个值是0；第二个是start+1*step，将上面的数代入，应该是1,但是最后一个还是-9，好像出现问题了。但是，数学这么严谨的学科是不能随便用“好像”“应该”这样的字眼。。再说了，The Zen of Python 都说了 Errors should never pass silently. 让我们大胆试验，看看 range()到底是怎么运行的。请看下面代码实验、结果和注释。：

 >>> range(0,1,1)    # step > 0，且start<stop为True, 所以有值返回。
[0]
>>> range(0,-1,1)   # step > 0，但start<stop为false, 所以返回空值。
[]
>>> range(0,0,1)    #step > 0 , 但start<stop为false，所以返回空值。
[]
>>> range(1,0,-1)   # step < 0，且start>stop为True, 所以有值返回。
[1]
>>> range(0,1,-1)   # step < 0，但start>stop为false, 所以返回空值。
[]
>>> range(0,0,-1)	#step < 0 , 但start>stop为false， 所以返回空值。
...
由此可见，内置函数range()的逻辑是要先判断(step>0) and (start<stop)  或者(step<0) and (start>stop)是否为真。如果为真，才会有值返回（至少有start这个值）。否则返回空值（因为条件不满足，后面的代码它就不执行了，包括填入start这个值也不会执行，所以是空列表，毕竟它至少要返回一个列表）。

而当 stop结束值没有给定，或step=0，也就是违反了函数参数的要求，所以会报错。例子如下
另外，记得，参数必须是整数，所以像0.2等有小数点的也都会报错。

>>> range()   #stop没有给定，违反了参数要求，会报错
